...
#         self.ui.setupUi(self)
    ...
    def final_report_data(self, offsprings):
        ...
        try:
            ...
            '''if min(self.ind_order.values()) == 1:
                tmp_gtypes.append('')
                tmp_theta.append('nan')
                tmp_r.append('nan')'''
            last_snp = None
            with open(self.args.finalreport) as fin:
                first_line = True
    ...
    #                         print last_snp, gencall
                        '''if self.args.p1 == self.args.p2:
                            tmp_gtypes[0] = tmp_gtypes[1]
                            tmp_theta[0] = tmp_theta[1]
                            tmp_r[0] = tmp_r[1]'''
            ...
            try:
                ...
                if export_states[snp[0]]:
                    ...
                    if out_type['gtypes']:
                        ...
                        if self.args.pop == 'Germplasm':
                            gtype_outf.write(','.join(snp[:3] + snp[3:11] + list(genos)[2:]) + '\n')
                        else:
